...
class PaymentScreenState extends State<PaymentScreen> {
  ...
  void initState() {
    ...
    Future.delayed(const Duration(milliseconds: 100), () {
      // Initialize or open your plugin-dependent view here.
      _initData();
    });
  }
  ...
  void _initData() async {
    ...
    browser = MyInAppBrowser(
        ...
        isSubscriptionPayment: widget.isSubscriptionPayment,
...
class MyInAppBrowser extends InAppBrowser {
  ...
  void _redirect(
      String url, int? storeId, bool? isSubscriptionPayment, int? packageId) {
    if (kDebugMode) {
      print('---url---$url');
    }
    if (_canRedirect) {
          ...
          Get.toNamed(RouteHelper.getSuccessRoute(
              isSuccess
                  ...
                  : isFailed
                      ? 'fail'
                      : 'cancel',
              isWalletPayment: url.contains(
                      '${AppConstants.baseUrl}/success?flag=success') ||
                  url.contains('${AppConstants.baseUrl}/success?flag=fail') ||
                  url.contains('${AppConstants.baseUrl}/success?flag=cancel')));
        }
      }
